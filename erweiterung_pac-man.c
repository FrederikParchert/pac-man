...
// Richtung von Pac-Man (dx, dy) für Bewegungen
int pacman_dx = 1; // Standardbewegung nach rechts
int pacman_dy = 0; // Standardbewegung nach rechts (keine vertikale Bewegung)

// Spielfeld für Level 1 initialisieren
void initializeGrid() 
{
...
void drawGrid() {
    ...
    printf("Score: %d\n", score);
    printf("Lives: %d\n", lives); // Leben anzeigen
    printf("Level: %d\n", level); // Aktuelles Level anzeigen
}
...
void movePacman() 
{
    int newX = pacman.x + pacman_dx;
    int newY = pacman.y + pacman_dy;
    ...
    if (grid[newY][newX] == 'G') 
    {
        ghostContactCount++; // Geisterkontakt zählen
        if (ghostContactCount >= 3) 
        {
            lives--; // Leben abziehen, wenn 3 Geisterkontakte erreicht sind
            ghostContactCount = 0; // Zurücksetzen des Zählers
            if (lives <= 0) 
            {
                running = false; // Spiel beenden, wenn keine Leben mehr übrig sind
            }
        }
        return;
    }
...
// Eingabe verarbeiten und Richtung von Pac-Man ändern
void handleInput() 
{
    if (kbhit()) 
    {
        ...
        switch (input) 
        {
            case 'w': pacman_dx = 0; pacman_dy = -1; break; // Hoch
            case 's': pacman_dx = 0; pacman_dy = 1; break;  // Runter
            case 'a': pacman_dx = -1; pacman_dy = 0; break; // Links
            case 'd': pacman_dx = 1; pacman_dy = 0; break;  // Rechts
            case 'l': running = false; break;   // Spiel beenden
        }
    }
}
...
int main() 
{
    ...
    while (running) 
    {
        initializeGrid(); // Grid für jedes Level initialisieren
        score = 0; // Punkte für das neue Level zurücksetzen
        ghostContactCount = 0; // Geisterkontakte zurücksetzen
        
        while (running) 
        {
            drawGrid();
            handleInput(); // Eingaben des Spielers verarbeiten
            ...
            {
                printf("You Won the Level %d!\n", level);
                level++; // Zum nächsten Level wechseln
                break; // Level wechseln
            }
